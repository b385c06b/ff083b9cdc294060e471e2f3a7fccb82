...
void swizzle_swap(Image * pimage)
{
	...
	swizzle_fast(buffer, (unsigned char *)pimage->data, pimage->textureWidth * 4, pimage->textureHeight);
	if (pimage->data)
	{
		free(pimage->data);
	}
...
Image* loadImage(const char* filename)
{
	...
	if (isJpegFile(filename)) {
		pimage = loadJpegImage(filename);
	} else {
		pimage = loadPngImage(filename);
	}
