...
int		m_breakOnRealloc(void *reportedAddress);
int		m_breakOnDealloc(void *reportedAddress);
...
int		m_ValidateMemory(const sAllocUnit *allocUnit);
int		m_validateAddress(const void *reportedAddress);
int		m_validateAllocUnit(const sAllocUnit *allocUnit);
int		m_validateAllAllocUnits();
...
#define	calloc(sz,c)			m_allocator  (__FILE__,__LINE__,__FUNCTION__,m_alloc_calloc,sz*c,1)
